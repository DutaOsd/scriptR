-- // Obby Helper Pro v3.2 - FULLY FIXED
-- // FIXED: Function order, Connection type, ESP throttle,
-- //        Color overflow, deprecated spawn, memory leaks
...
-- ════════════════════════════════════════════
-- CONNECTION MANAGER (FIXED: Unified type)
-- ════════════════════════════════════════════
...
local SPEED_SETTINGS = {
    ...
    ShowOthers  = true,
    MaxSpeed    = 100,
    UpdateRate  = 0.05,   -- FIXED: 20x/s bukan unlimited
...
local ToggleStates = {
    ...
    InfJump   = false, Noclip    = false, AntiVoid = false, Fullbright = false,
    AntiAFK   = false, AutoJump  = false,
}

local oldLighting = {}

-- FIXED: Timer throttle untuk ESP & speed
local espTimer   = 0
local speedTimer = 0
...
local function createSlider(labelText, parent, minVal, maxVal, default, suffix, callback, order)
    ...
    -- FIXED: Stored in ConnectionManager
    local sliderConnName = "Slider_" .. labelText
    Connections:Add(sliderConnName, UserInputService.InputChanged:Connect(function(input)
...
-- ════════════════════════════════════════════
-- SPEED UPDATE LOOP (FIXED: Throttled)
-- ════════════════════════════════════════════
                ...
                if localHRP then
                    ...
                    if dist > ESP_SETTINGS.FadeStart then
                        fadeAlpha = 1 - math.clamp(
                            (dist - ESP_SETTINGS.FadeStart) / (ESP_SETTINGS.MaxDistance - ESP_SETTINGS.FadeStart),
                            0, 1
                        )
                    end
                end
...
local function createPlayerESP(plr)
    ...
    -- FIXED: pcall wrapping dengan warn
    ...
    if not ok then
        warn("[ObbyHelper ESP] Error for " .. plr.Name .. ": " .. tostring(err))
    end
end

-- FIXED: Throttled ESP update (20x/detik bukan tiap frame)
    ...
    for plr, data in pairs(ESP) do
        local ok = pcall(function()
            ...
            if distance > ESP_SETTINGS.FadeStart then
                fadeAlpha = 1 - math.clamp(
                    (distance - ESP_SETTINGS.FadeStart) / (ESP_SETTINGS.MaxDistance - ESP_SETTINGS.FadeStart),
                    0, 1
                )
            end
            ...
            if data.InfoLabel then
                ...
                -- FIXED: Tidak kali 255 lagi, langsung clamp 0-1
...
-- ════════════════════════════════════════════
-- MAIN HEARTBEAT LOOP (FIXED: satu loop untuk semua)
-- ════════════════════════════════════════════
    ...
    if ToggleStates.Obstacle then
        ...
        for _, part in ipairs(Workspace:GetDescendants()) do
            pcall(function()
                ...
                and part.Transparency >= 0.85 and not ObstacleFolder[part] then
        ...
        Connections:Add("AntiVoid", RunService.Heartbeat:Connect(function()
            pcall(function()
                ...
                if hrp.Position.Y < -80 then
                    hrp.CFrame = lastSafe
                        and (lastSafe + Vector3.new(0, 10, 0))
...
AutoJumpToggle.MouseButton1Click:Connect(function()
    ...
    if ToggleStates.AutoJump then
        Connections:Add("AutoJump", RunService.Heartbeat:Connect(function()
            pcall(function()
                local h = player.Character and player.Character:FindFirstChildOfClass("Humanoid")
...
-- ════════════════════════════════════════════
-- PLAYERS TAB FUNCTIONS (FIXED: Defined before use)
-- ════════════════════════════════════════════
createWarningBox(
...
StopAllBtn.MouseButton1Click:Connect(function()
    stopAllActions()
    showNotif("🛑 Stopped", Color3.fromRGB(60, 30, 30))
end)

-- Player Entry Functions (FIXED: spawn -> task.spawn + exit condition)
...
local function createPlayerEntry(t, idx)
    ...
    nL.Size               = UDim2.new(1, -(oX + 5), 0, 20)
    nL.Position           = UDim2.new(0, oX, 0, 6)
    ...
    hL.Size               = UDim2.new(1, -(oX + 5), 0, 16)
    hL.Position           = UDim2.new(0, oX, 0, 24)
    ...
    -- FIXED: task.spawn + proper exit condition
    task.spawn(function()
                ...
                if t.Character then
                    ...
                    if hum and hrp then
                        ...
                        if SpeedData[t] and SpeedData[t].speed then
                            speedText = string.format(" • ⚡%.0f st/s", SpeedData[t].speed)
                        end
...
-- FIXED: refreshPlayerList sekarang defined sebelum digunakan di tab buttons
...
-- Tab button connections (FIXED: refreshPlayerList sudah ada)
...
-- ════════════════════════════════════════════
-- SERVER TAB (FIXED: Better error handling)
-- ════════════════════════════════════════════
...
-- FIXED: Separate pcall untuk HttpGet dan JSONDecode
local function safeHttpGet(url)
    local httpOk, response = pcall(function()
        return game:HttpGet(url)
    end)
    if not httpOk then
        warn("[ObbyHelper] HttpGet failed: " .. tostring(response))
        return nil
    end
    ...
    if not jsonOk then
        warn("[ObbyHelper] JSONDecode failed: " .. tostring(decoded))
        return nil
    end
...
RandomBtn.MouseButton1Click:Connect(function()
    ...
    if #avail == 0 then
        showNotif("⚠️ Load servers first!", Color3.fromRGB(80, 50, 20))
        return
    end
...
mkInfo(
    ...
    "<font color='#FF3C3C'>⚡ 100+ st/s = Extreme</font>\n\n" ..
    "• YOU badge = kecepatan diri sendiri\n" ..
...
mkInfo(
    ...
    "<b>Fix v3.2F:</b> Memory leak, Color bug, Throttle, spawn→task",
    InfoContent, 6
)
...
CloseBtn.MouseButton1Click:Connect(function()
    ...
    -- FIXED: ConnectionManager RemoveAll disconnect semuanya
...
task.delay(0.8, function()
    showNotif("🔥 Obby Helper v3.2 Ready!", Color3.fromRGB(30, 40, 80))
end)

print("╔════════════════════════════════════╗")
print("║  🔥 Obby Helper Pro v3.2 FIXED     ║")
print("║  ✅ Connection Manager             ║")
print("║  ✅ ESP Throttle Fix               ║")
print("║  ✅ Color Overflow Fix             ║")
print("║  ✅ spawn → task.spawn             ║")
print("║  ✅ Function Order Fix             ║")
